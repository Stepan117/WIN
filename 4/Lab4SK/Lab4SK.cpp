...
DWORD WINAPI Thread1(LPVOID t) {
	while (true) {
		...
		FillRect(hdc, &Window, CreateSolidBrush(RGB(128, 128, 128)));
		DrawText(hdc, L"Технический перерыв", 20, &Window, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
	}
	return 0;
}
...
DWORD WINAPI Thread2(LPVOID t) {
	...
	case 0:
	{
		FillRect(hdc, &Window, CreateSolidBrush(RGB(48, 213, 200)));
		ReleaseDC((HWND)t, hdc);
		EndPaint((HWND)t, &ps);
		return 0;
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		AppendMenu(hMenu, MF_STRING | MF_POPUP, 1002, L"Случайное");
		HMENU hMenu1 = CreateMenu();
		AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hMenu1, L"Технический перерыв");
		AppendMenu(hMenu1, MF_STRING | MF_POPUP, 1000, L"Начать");
		AppendMenu(hMenu1, MF_STRING | MF_POPUP, 1001, L"Закончить");
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR szCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindow(wc.lpszClassName, L"Lab_4", WS_OVERLAPPEDWINDOW, 0, 0, 500, 800, NULL, NULL, wc.hInstance, NULL);
