...
/* Задание B13. Сформировать два приложения, которые открывают по одному окну. В окне 1 по щелчку левой клавиши мыши (при помощи
   ...
   WPARAM  передать свой дескриптор второму окну. Выдать сообщение об этом. В окне 2: при получении 
...
TCHAR WinName[] = _T("Второе окно"); // Имя окна
//————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) { // Оконная функция (Функция обратного вызова)
	...
	case WM_USER + 1: // Сообщение переданное первым окном
		brush = CreateSolidBrush(RGB(rand() % 256, rand() % 256, rand() % 256)); //Заполнение окна цветом 
	...
	case WM_USER +2:
		ShowWindow(hWnd,SW_MAXIMIZE); //Открытие окна во весь экран
		break;
